const websocket = io("/", { path: "/avGetter" });
let ttlList = {}

function send() {
    const avWeb = $("#avWeb").val();
    const avUrl = $("#avUrl").val();

    switch (avWeb) {
        case "jable":
            if (!avUrl.startsWith("https://jable.tv/")) {
                alert("網址錯誤")
                return false;
            }
            break;
...
function batchWork() {
    ...
    ,
          {
            avWeb: $("#avWeb").val(),
            avUrl: "https://www.dm5.cn/manhua--dongbeijunzisige-hejunjiangyiqi/"
          }
    */
